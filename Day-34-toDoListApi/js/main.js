...
var renderTodos = function() {
    ...
    const html = todos.map(function(todo, index) {
        var value = formUpdateIds.find(function(el) {
            return +el.index === +index;
        });
        if(value) {
            return `
               <form class="TodoForm">
                  <input type="text" class="todo-input" placeholder="Update task"
                      value="${value.name}">
        ...
        <div class="Todo" data-index="${index}">
            <p data-type="completed" class="${todo.completed ? "completed" : ""}">${todo.name}</p>
            <div>
...
var showFormUpdate = function(todoEl) {
    ...
    // console.log(todo);
    var formUpdate = `
         <form class="TodoForm" data-index="${index}">
            <input type="text" class="todo-input" placeholder="Update task"
                value="${todo.name}">
            <button type="submit" class="todo-btn">Add Task</button>
        </form>
    ...
    formList.forEach(function(todoForm) {
        if(todoForm.dataset.index === index) {
            ...
            todoForm.querySelector(".todo-input").addEventListener("change", function() {
                value = this.value;
               
                var indexUpdate = formUpdateIds.findIndex(function(el) {
                    return +el.index === +index;
                });
                if(indexUpdate !== -1) {
                ...
                todoForm.addEventListener("submit", function(e) {
                    ...
                    formUpdateIds = formUpdateIds.filter(function(el) {
                        return +el.index !== +index;
                    })
                });
            })
