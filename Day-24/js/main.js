...
var taskList = $(".task-group");
var tasks = [];
btnAction.addEventListener("click", function () {
    var taskId = this.getAttribute("id");
    if (taskId == 0 || taskId) {
        tasks[taskId] = { name: mainInput.value };
        this.removeAttribute("id");
    } else {
        tasks.push({ name: mainInput.value });
    }
    mainInput.value = "";
    renderTasks(tasks);
});
...
var editTask = function (id) {
    mainInput.value = tasks[id].name;
    btnAction.setAttribute("id", id);
};

var deleteTask = function (id) {
    tasks.splice(id, 1);
    //delete object with id and number
    renderTasks(tasks);
};
...
var renderTasks = function (tasks = []) {
    ...
    tasks.forEach(function (task, index) {
        content += `<div class="task">
            ...
            <div class="task-fix" onclick="editTask(${index})">
